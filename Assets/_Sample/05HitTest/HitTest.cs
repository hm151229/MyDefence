...
namespace Sample
{
    ...
    public class HitTest : MonoBehaviour
    {
        ...
        private bool CheckHitCircle(cCircle a, cCircle b)
        {
            ...
            //두 원 사이의 거리가 두 원의 반지름의 합보다 작으면 충돌이라고 판정
            float distanece = Mathf.Sqrt(xDistance * xDistance + yDistance * yDistance);
            if (distanece <= (a.r + b.r)) 
            {
                return true;
            }
            return false;
        ...
        //도착 판정으로 충돌체크
        //두 오브젝트의 거리가 일정거리안에(0.5f) 있으면 충돌이라 판정
        //충돌했으면 true, 충돌하지 않았으면 false 변환
        ...
        //이동 시 타겟까지의 남은 거리가 한 프레임에 이동하는 거리보다 작을 때 충돌이라고 판정
